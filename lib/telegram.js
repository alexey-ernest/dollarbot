 ...
 * @param      {boolean}   isInline  Indicates whether to send reply to the chat inline.
 * @param      {Function}  fn        Callback function: function (err) {}
 */
Telegram.prototype.send = function (text, chatId, isInline, fn) {
  if (typeof isInline === 'function') {
    // swap arguments
    fn = isInline;
    isInline = false;
  }
  ...
  debug('Sending to ' + chatId + ' message: ' + text + (isInline ? ' (inline)' : ''));
  ...
  if (!isInline) {
    ...
    url = API + this.token + '/answerInlineQuery';
    var article = {
      type: 'article',
      id: uuid.v4(),
      title: text,
